...
if __name__ == '__main__':
    ...
    if len(str(args.year)) == 4:
        ...
        logging.info(
            ...
            str(args.year), int(args.minutes), str(Path(args.work_dir))))
        interval = 10
        while True:
            try:
                logging.info('STEP 1/3 downloading AIS data')
                # download AIS data
                download_AIS(args.year, args.work_dir)
                break
        ...
        while True:
            try:
                ...
                subsample_AIS_to_CSV(args.year, args.work_dir, args.minutes)
                break
            except Exception as e:
                logging.error(traceback.format_exc())
        ...
        while True:
            try:
                ...
                append_environment_data(args.year, args.minutes, args.work_dir)
                break
            except Exception as e:
                logging.error(traceback.format_exc())
